...
test.describe('END-TO-END WORKFLOWS - Test Suite', () => {
    ...
    test('TC-E2E-008: Different addresses return different coordinates', async () => {
        const addresses = [
    ...
    test('TC-E2E-009: Complete workflow stress test - Multiple searches and selections', async () => {
        // Login
        await loginPage.navigateToLogin();
        await loginPage.login(config.credentials.username, config.credentials.password);
        await loginPage.waitForURL('**/search.html');
